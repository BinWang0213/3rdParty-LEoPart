...
from dolfin import (
    ...
    assemble,
    dx, dS, ds,
    dot, inner, outer, TestFunction, CellDiameter, FacetNormal, derivative,
PETScMatrix, PETScVector, PETScKrylovSolver, PETScOptions
)
import numpy as np
from mpi4py import MPI as pyMPI
from leopart import (
    StokesStaticCondensation, FormsStokes, AssemblerStaticCondensation)
import pytest
...
def test_steady_stokes(k):
    ...
    if comm.rank == 0:
        ...
        assert np.all(conv_u > k + 0.75)
        assert np.all(conv_p > (k - 1) + 0.75)
