...
def _get_dolfin_function_cpp_obj(obj):
    obj = obj._cpp_object if isinstance(obj, dolfin.Function) \
        else obj
    return obj
...
class particles(compiled_module.particles):
    ...
    def interpolate(self, *args):
        ...
        """
        a = map(_get_dolfin_function_cpp_obj, args)
    ...
    def increment(self, *args):
        ...
        The increment from the current and previous time steps may also be
        implemented via the overloaded method

        .. code-block:: python

            #  Incremental update with  theta =0.5, step=2
            p.increment(dpsih_dt, [1, 2], theta, step, dt)

        where dpsih_dt is the rate of change with respect to time in the
        diffusive component of the solution and dt is the current time step.
...
class AssemblerStaticCondensation(compiled_module.AssemblerStaticCondensation):
    """
    Class facilitating HDG static condensation assembly.
    Class interfaces with the cpp AssemblerStaticCondensation class
    """
    ...
    def __init__(self, *args):
        ...
        def to_dolfin_cpp_form(a):
            ...
            return a
        args = list(map(to_dolfin_cpp_form, args))
        super().__init__(*args)
